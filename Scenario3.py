...
class CourierAgent:
    ...
    def available_actions(self, current_package: int, state: int) -> [int]:
        ...
        actions = []
        for idx, element in enumerate(self.reward_matrix[current_package, state]):
            if element != -1:
                actions.append(idx)
        return actions
    ...
    def calculate_reward_matrix(self) -> np.array:
        # Calculate the reward matrix for the environment
    ...
    def exploration_phase(self, episode: int, gamma: float = 0.9) -> None:
        # Exploration phase where agent learns through interactions
        ...
        while not goal_reached:
            ...
            if random.random() > self.epsilon:
                action = np.argmax(q_table[current_package, state])
            else:
        ...
        if self.end_decay >= episode >= self.start_decay:
            self.epsilon -= self.decay_value

    def store_move_info(self, q_table, visit_table, grid_cell, remaining_packages, collected_packages, next_packages) -> (np.array, np.array, int, int):
        # Store move information and update tables
    ...
    def exploitation_phase(self) -> None:
        # Exploitation phase where agent exploits learned knowledge
        x, y = self.room.getPosition()
        goal_reached = False
    ...
    def learning_phase(self) -> np.array:
        # Learning phase where agent learns from interactions
        ...
        self.reward_matrix = self.calculate_reward_matrix()
        ...
        for episode in range(1, self.episodes):
            self.exploration_phase(episode)
...
def main():
    ...
    # Create agent
    agent = CourierAgent(room)
    print("Training starting...")
    # Train the agent
    agent.learning_phase()
    # Evaluate the agent
